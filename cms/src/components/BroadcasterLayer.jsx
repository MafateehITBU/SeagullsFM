...
const BroadcasterLayer = () => {
  ...
  const fetchBroadcasters = async () => {
    try {
      ...
      const allBroadcasters = res.data.data || [];
      const filtered = allBroadcasters.filter(
        (b) => b.channelId?._id === user?.channelId
      );
  ...
  const totalPages = Math.max(1, Math.ceil(broadcasters.length / pageSize));
  const paginatedBroadcasters = useMemo(() => {
    const start = (page - 1) * pageSize;
    return broadcasters.slice(start, start + pageSize);
  }, [broadcasters, page]);
  ...
  const {
    ...
    { columns, data: paginatedBroadcasters },
    useGlobalFilter,
    useSortBy
  );

  const noResultsAfterSearch =
    !loading && broadcasters.length > 0 && rows.length === 0;
  ...
  return (
    <div className="card basic-data-table" style={{ minHeight: "65vh" }}>
      ...
      <div className="card-header d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
        <h5 className="card-title mb-0">Broadcasters</h5>
...
export default BroadcasterLayer;
